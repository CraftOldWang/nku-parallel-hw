...
const ExperimentConfig EXPERIMENTS[] = {
    ...
    {15000000, 1000000, "数据集/15M测试"},
    {20000000, 1000000, "数据集/20M测试"},
    {25000000, 1000000, "数据集/25M测试"},
};
        ...
        try {
            ...
            while (!q.priority.empty())
            {
                ...
                if (iterations % 1000 == 0) {
                    cout << "进度: 迭代 " << iterations << ", 当前猜测数: " << q.total_guesses 
                         << ", 活动任务组: " << get_active_task_groups_count() << endl;
                }
                ...
                try {
                    ...
                    if (global_async_thread_pool) {
                        ...
                        // 更保守的控制策略
                        if (active_groups > 16 || queue_size > 200) {
                            usleep(10000); // 10ms等待
                    ...
                    // 检查内存使用情况，防止内存不足
                    if (q.total_guesses > 10000000) {  // 降低到1000万
